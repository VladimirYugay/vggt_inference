...
if __name__ == '__main__':
    ...
    for scene in scannet_scenes:
        ...
        dataset = TUM_RGBD(scene_dir)
        c2ws = np.array([dataset[i][2] for i in range(len(dataset))])
        ...
        print("Speed", len(all_cam_to_world_mat), time.time() - start_time)
        ...
        np.save(output_scene_dir / "pred_traj.npy", traj_est_poses)
        if args.plot:
